...
- PDF export REALDOC-E.4 binds the protected canonical record to source-neutral
  ...
  Editor pre-test implementation continues with E.5.6 through E.5.9 after the
  accepted E.5.1 Library, E.5.2 workspace tabs, E.5.3 Core projection, and
  E.5.4 temporary generated Form plus E.5.5 JSON/mapping preparation:
...
- PDF export REALDOC-E.5.0 locks the Editor product surface without runtime
  ...
  URL-backed Design and Preview views; generated Form and mapped JSON input
  remain temporary, Draft and Published Preview retain distinct identities,
  and stale results cannot masquerade as current artifact truth:
  `../flowdoc-vnext-editor/docs/REALDOC_DOCUMENT_WORKSPACE_PRODUCT_CONTRACT.md`.
- PDF export REALDOC-E.5.1 accepts the bounded local Backend Library query and
  metadata route plus the Editor `/documents` to Design navigation. The list is
  explicitly not user-authorized, excludes raw package/generated content, and
  leaves Preview and production inactive:
  `../flowdoc-vnext-editor/docs/REALDOC_DOCUMENT_LIBRARY.md`.
- PDF export REALDOC-E.5.2 accepts one document-keyed Editor runtime with a
  shared header and URL-backed Design/Preview tabs. Design state survives view
  changes; Preview reports unavailable and calls no generation/artifact route:
  `../flowdoc-vnext-editor/docs/REALDOC_DOCUMENT_WORKSPACE_TABS.md`.
- PDF export REALDOC-E.5.3 accepts one pure Core test-input projection. It
  derives first-placement order, section groups, collection item fields, and
  image/media requirements from exact Published Structure contracts while
...
- PDF export REALDOC-E.5.5 accepts Editor-owned memory-only JSON text, exact
