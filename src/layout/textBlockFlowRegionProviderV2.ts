...
export function provideVNextTextBlockFlowRegionsV2(input: unknown): VNextTextBlockFlowRegionResultV2 {
  ...
  if (inspectVNextTextBlockSpatialIndexV2(acceptedInput.spatialIndex).status !== "valid") return blocked([issue("spatial-index-provenance-mismatch", "spatialIndex", "flow region provider requires a process-local V2 index")])
  if (!hasSpatialIndexBindingV2({ initialFlow: acceptedInput.initialFlow, evidence: acceptedInput.evidence, persistentFlowTree: acceptedInput.persistentFlowTree, index: acceptedInput.spatialIndex })) return blocked([issue("spatial-index-binding-mismatch", "spatialIndex", "flow region provider requires the exact V2 authority-bound index")])
  if (!Number.isSafeInteger(acceptedInput.band.topLayoutUnit) || !Number.isSafeInteger(acceptedInput.band.bottomLayoutUnit) || acceptedInput.band.topLayoutUnit < 0 || acceptedInput.band.bottomLayoutUnit <= acceptedInput.band.topLayoutUnit) return blocked([issue("invalid-line-band", "band", "flow region provider requires a non-negative safe half-open line band")])
  if (!Number.isSafeInteger(acceptedInput.contentInsets.leftLayoutUnit) || !Number.isSafeInteger(acceptedInput.contentInsets.rightLayoutUnit)) return blocked([issue("unsafe-region-arithmetic", "contentInsets", "content insets must use safe integer layout units")])
  if (acceptedInput.contentInsets.leftLayoutUnit < 0 || acceptedInput.contentInsets.rightLayoutUnit < 0) return blocked([issue("invalid-content-insets", "contentInsets", "content insets must be non-negative")])
  const start = acceptedInput.spatialIndex.contentLeftLayoutUnit + acceptedInput.contentInsets.leftLayoutUnit
  const end = acceptedInput.spatialIndex.contentRightLayoutUnit - acceptedInput.contentInsets.rightLayoutUnit
  ...
  const kernel = computeVNextTextBlockFlowRegionKernelV1({ contentStartLayoutUnit: start, contentEndLayoutUnit: end, bandTopLayoutUnit: acceptedInput.band.topLayoutUnit, bandBottomLayoutUnit: acceptedInput.band.bottomLayoutUnit, flowAffectingEntryCount: acceptedInput.spatialIndex.summary.flowAffectingEntryCount, query: () => queryVNextTextBlockSpatialIndexV2(acceptedInput.spatialIndex, acceptedInput.band) })
  if (kernel.status === "blocked") return blocked([failureIssue(kernel.failure)])
  const facts = { status: "accepted" as const, source: "vnext-text-block-flow-region-v2" as const, contractVersion: 2 as const, spatialIndexFingerprint: acceptedInput.spatialIndex.fingerprint, band: { ...acceptedInput.band }, contentInsets: { ...acceptedInput.contentInsets }, intervals: kernel.intervals.map((item) => ({ ...item })), intersectingEntryFingerprints: [...kernel.intersectingEntryFingerprints], nextYLayoutUnit: kernel.nextYLayoutUnit, work: { ...kernel.work }, mayPublishLayout: false as const, productionBinding: false as const, issues: [] as [] }
