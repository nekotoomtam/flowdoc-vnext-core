...
describe("internal alpha close audit and documentation consolidation gate", () => {
  ...
  it("adds compact current-state documentation without deleting the audit trail", () => {
    ...
    expect(currentStatus).toContain("Status: updated after Phase 181.")
    expect(currentStatus).toContain("Use this file first when orienting current work.")
    expect(currentStatus).toContain("Phase 181: Internal Alpha Close Audit And Documentation Consolidation Gate.")
    expect(currentStatus).toContain("Phase 182: V1 Hardening Backlog Triage Gate.")
    ...
    expect(nextPointer).toContain("Status: current after Phase 181.")
    expect(nextPointer).toContain("Phase 182: V1 Hardening Backlog Triage Gate.")
    expect(nextPointer).toContain("ranked hardening backlog")
  })

  it("recommends Phase 182 as a triage gate, not a production binding jump", () => {
    const audit = readText("../docs/INTERNAL_ALPHA_CLOSE_AUDIT_AND_DOC_CONSOLIDATION_GATE.md")
    const pointer = readText("../docs/NEXT_PHASE_POINTER.md")
    ...
    expect(pointer).toContain("choose a")
    expect(pointer).toContain("single first production hardening lane")
    expect(pointer).toContain("No production contenteditable implementation.")
    expect(pointer).toContain("No backend route/server/auth/authz implementation.")
    expect(pointer).toContain("No production PDF/DOCX renderer.")
    expect(pointer).toContain("No default measurement replacement.")
  })
  ...
  it("documents Phase 181 in the README, roadmap, and ledger", () => {
    ...
    expect(roadmap).toContain("Current next step after Phase 181:")
