...
describe("vNext rich inline live/exact parity audit", () => {
  ...
  it("records the Phase 130 audit trail without overstating runtime scope", () => {
    ...
    expect(persistenceSource).toContain("liveLayout: false")
    expect(persistenceSource).toContain("exactLayout: false")
    expect(persistenceSource).toContain("artifacts: false")
